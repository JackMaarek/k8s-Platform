# domains/platform / dev

Platform domain infrastructure for **dev** — node groups and IRSA roles for platform workloads.

Reads cluster foundation from `_core/shared/dev` via `terraform_remote_state`. Never re-declares VPC, EKS, or shared IAM resources.
...
|---------|-------|
| Cluster | `dev-k8s` |
| Shared state | `core/shared/dev/terraform.tfstate` |
...
Or via platform-bot:
```bash
platform-bot nodegroup add --domain platform --env dev --name quantum --type c5.2xlarge
platform-bot nodegroup remove --domain platform --env dev --name gpu
```

## Apply

> Always apply `_core/shared/dev` first.
...
| `shared.tf` | `terraform_remote_state` + locals from `_core/shared/dev` |
