...
# This module only adds staging-specific account assignments.
#
# Access matrix for staging:
#   platform-devs        → readonly
#   platform-maintainers → platform-maintainer (poweruser with prod-deny)
...
module "identity_center_assignment" {
  ...
  permission_set_arn_maintainers = data.terraform_remote_state.shared_dev.outputs.permission_set_arn_platform_maintainer
